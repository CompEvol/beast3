...
public class SpeciesTreePrior extends SpeciesTreePopFunction {
    ...
    public final Input<RealScalar<PositiveReal>> gammaParameterInput = new Input<>("gammaParameter", "scale parameter of the gamma distribution over population sizes. "
    ...
    public void initAndValidate() {
        ...
        final RealScalarParam<PositiveReal> parameter = new RealScalarParam<>(4.0, PositiveReal.INSTANCE);
...
//                    logP += gamma2Prior.calcLogP(popSizesBottom);
                    for (int i = 0; i < popSizesBottom.size(); i++) {
                        final double popSize = popSizesBottom.get(i);
                        logP += gamma2Prior.logDensity(popSize);
                    }
