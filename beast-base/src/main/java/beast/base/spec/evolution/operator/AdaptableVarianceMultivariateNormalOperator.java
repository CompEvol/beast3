...
//        this.updates = 0;
	...
	public List<StateNode> listStateNodes() {
		...
		for (Transform t : transformations) {
			...
			if (this.allowNonsense) {
				int fDim = 0;
				for (Tensor<?,?> f : t.getF()) fDim += f.size();
...
//            this.updates = lowestNumberOfSamples;
    ...
    public class CompoundParameterHelper {
        ...
        /** list of Tensors and Trees **/
        final List<Object> parameterList;

        public CompoundParameterHelper(final List<Object> parameterList) {
            ...
            int dim = 0;
            for (final Object para : parameterList) {
            	if (para instanceof Tensor t) {
            		dim += t.size();
            ...
            for (int y = 0; y < parameterList.size(); y++) {
                final Object para = parameterList.get(y);
            	if (para instanceof Tensor t) {
        ...
        public int setValue(final int param, final double value) throws Exception {
            ...
            final Object para = parameterList.get(getY(param));
            if (para instanceof RealScalarParam p) {
            	if (value > p.getUpper()) {
            		p.set(p.getUpper());
            	...
            	return 1;
            } else if (para instanceof RealVectorParam p) {
            	if (value > p.getUpper()) {
            		p.set(getX(param), p.getUpper());
            	} else if (value < p.getLower()) {
            		p.set(getX(param), p.getLower());
            	} else {
            		p.set(getX(param), value);
            	}
            	return 1;
            } else if (para instanceof IntScalarParam p) {
            	if (value > p.getUpper()) {
            		p.set(p.getUpper());
            	} else if (value < p.getLower()) {
            		p.set(p.getLower());
            	...
            	return 1;
            } else if (para instanceof IntVectorParam p) {
            	if (value > p.getUpper()) {
            		p.set(getX(param), p.getUpper());
            	} else if (value < p.getLower()) {
            		p.set(getX(param), p.getLower());
        ...
        public double getValue(final int param) {
        	Object f = parameterList.get(getY(param));
        	
            if (f instanceof RealScalarParam p) {
            	return p.get();
            } else if (f instanceof RealVectorParam p) {
            	return p.get(getX(param));
            } else if (f instanceof IntScalarParam p) {
            	return p.get();
