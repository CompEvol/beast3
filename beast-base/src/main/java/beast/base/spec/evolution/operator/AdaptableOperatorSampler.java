...
public class AdaptableOperatorSampler extends Operator {
    ...
    final public Input<List<Tensor<?,?>>> paramInput = new Input<>("parameter", "list of parameters to compare before and after the proposal. If the tree heights"
    		+ " are a parameter then include the tree under 'tree'", new ArrayList<>());
			...
			if (this.treeMetric != null) {
				...
				for (int p = 0; p < this.numParams; p ++) {
					...
					if (trees != null && p == this.numParams - 1 ) {
						...
						// A RealParameter
						weight += this.parameters.get(p).size();
					}
	...
	private double[][] getAllParameterValues() {
		...
		for (int p = 0; p < this.numParams; p ++) {
			...
			if (trees != null && p == this.numParams - 1 ) {
				...
				// A tensor
				Tensor<?,?> t = this.parameters.get(p);
				if (t instanceof RealScalar r) {
					p_vals = new double[] {r.get()};
				} else if (t instanceof IntScalar r) {
					p_vals = new double[] {r.get()};
				} else if (t instanceof RealVector r) {
					List list = r.getElements();
					p_vals = new double[list.size()];
					for (int i = 0; i < list.size(); i++) {
						p_vals[i] = (Double) list.get(i);
					}
				} else if (t instanceof IntVector r) {
					List list = r.getElements();
					p_vals = new double[list.size()];
					for (int i = 0; i < list.size(); i++) {
						p_vals[i] = (Integer) list.get(i);
					}
				} else {
...
//	       		if (s instanceof CompoundRealScalarParam c) {
//	       			stateNodes.remove(i);
//	       			
