...
public abstract class Parameter<T> extends StateNode {//implements Bounded<T> {
     ...
     * after the inputs, as usual) so that valuesInput can determine the class.
     */
...
//        storedValues = (T[]) Array.newInstance(getMax().getClass(), dimension);
        values = (T[]) Array.newInstance(genericClass, dimension);
        storedValues = (T[]) Array.newInstance(genericClass, dimension);

        /* That means if dimension is larger than valuesString.length,
           the values in valuesString will be repeated to fill the whole values array. */
...
//        return getKeysList().toArray(new String[0]);
//    }
...
//        buf.append("(").append(m_fLower).append(",").append(m_fUpper).append("): ");
    ...
    public Parameter<T> copy() {
        try {
            @SuppressWarnings("unchecked") final Parameter<T> copy = (Parameter<T>) this.clone();
    ...
    public void assignTo(final StateNode other) {
        @SuppressWarnings("unchecked") final Parameter<T> copy = (Parameter<T>) other;
        copy.setID(getID());
        copy.index = index;
...
//        copy.m_fUpper = m_fUpper;
    ...
    public void assignFrom(final StateNode other) {
        @SuppressWarnings("unchecked") final Parameter<T> source = (Parameter<T>) other;
        setID(source.getID());
...
//        m_fUpper = source.m_fUpper;
     ...
     * @return
     */
    public T getMatrixValue(final int row, final int column) {
        return values[row * minorDimension + column];
    }
...
//    }
    ...
    public T[] getRowValues(String key) {
