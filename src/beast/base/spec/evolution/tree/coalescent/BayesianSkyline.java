...
//public class BayesianSkyline extends PopulationFunction.Abstract {
    ...
    final public Input<RealVector<? extends PositiveReal>> popSizeParamInput = new Input<>("popSizes", "present-day population size. "
            ...
            Validate.REQUIRED);
    final public Input<IntSimplex<? extends PositiveInt>> groupSizeParamInput = new Input<>("groupSizes",
...
//			"The intervals of the tree containing coalescent node times for use in defining BSP.", Validate.REQUIRED);
    ...
    RealVector<? extends PositiveReal> popSizes;
    IntSimplex<? extends PositiveInt> groupSizes;
//    Tree tree;
...
//                parameter.setBounds(1, Integer.MAX_VALUE);
    ...
    public void prepare() {
        cumulativeGroupSizes = new int[groupSizes.size()];
    ...
    public List<String> getParameterIds() {
        ...
        if (popSizes instanceof BEASTInterface beastInterface)
            paramIDs.add(beastInterface.getID());
        if (groupSizes instanceof BEASTInterface beastInterface)
            paramIDs.add(beastInterface.getID());
    ...
    public double getPopSize(double t) {
        ...
        if (t > coalescentTimes[coalescentTimes.length - 1])
            return popSizes.get(popSizes.size() - 1);
        ...
        if (groupIndex >= popSizes.size()) {
            groupIndex = popSizes.size() - 1;
        }

        return popSizes.get(groupIndex);
    }
