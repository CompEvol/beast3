...
\item \textbf{First-found wins:} when the same JPMS module appears in multiple locations (boot layer, Maven package, ZIP package), the first one loaded wins. No silent duplicates, no ``reads more than one module'' errors.
...
Add \code{module-info.java} when you can: it is the primary mechanism and gives you compile-time module dependency checking.
\end{block}
\end{frame}
...
\item Modules with unsatisfied dependencies (e.g.\ FX on a headless cluster) are \textbf{automatically skipped}; core modules still load
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \code{maven-packages.xml}: installed coordinates
\item \code{maven-repo/}: local JAR cache (package + transitive deps)
\item Remote repos: Maven Central + extras in \code{beauti.properties}
\item Index needed because the cache is opaque
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\item \textbf{Scan package directories:} find installed ZIP packages in four locations (first match wins):
  \begin{itemize}
  \item \code{BEAST\_PACKAGE\_PATH} (env var or \code{-D} property)
  \item \code{\textasciitilde/.beast/2.8/} (user packages)
  \item System package dir (e.g.\ \code{/opt/beast/}, for cluster-wide installs)
  \item BEAST installation directory
  \end{itemize}
...
Packages can be published as plain Maven Central JARs.
...
\begin{itemize}
\item \code{maven-packages.xml}: records installed coordinates
\item \code{\textasciitilde/.beast/2.8/maven-repo/}: local JAR cache
\item Remote repos: Maven Central (always) + custom repos in \code{beauti.properties}
\end{itemize}

\vspace{0.5em}
\begin{itemize}
\item \code{packagemanager -addMavenRepository <url>}: add a custom repo
