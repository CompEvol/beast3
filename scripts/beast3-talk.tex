...
\begin{frame}{ModuleLayers replace custom classloader}
\begin{alertblock}{The custom classloader was an idiosyncratic barrier}
BEAST 2's plugin classloader predates Java's module system by 15+ years. It was the primary source of ``magic'' that confused new developers and defeated standard tooling.
\end{alertblock}

\vspace{0.5em}
\begin{itemize}
\item Using BEAST as a \textbf{library} in other projects (e.g.\ LPhyBEAST) was nearly impossible because the custom classloader conflicts with the host application's class loading
\item AI-assisted tools (Copilot, agentic coding agents) don't understand bespoke classloaders, but they do understand Maven projects and JPMS modules
\item Debugging class-not-found errors requires BEAST-specific knowledge that isn't transferable
\end{itemize}

\vspace{0.5em}
\textbf{BEAST 3 fix:} JPMS \code{ModuleLayer} does a better job out of the box, with per-package isolation that eliminates class conflicts the custom classloader can't deal with. Standard Java, standard tooling, no magic.
\end{frame}
...
\begin{frame}{Package Loading at Startup}
...
\item \textbf{Parse \code{version.xml}} --- read service declarations and dependency info from each package
\item \textbf{Create ModuleLayer per package:}
  \begin{itemize}
  \item Discover JPMS modules from the package's JARs
  \item Exclude modules already in boot layer (no duplicates)
  \item Filter out modules whose \code{requires} can't be satisfied
  \item \code{resolveAndBind()} on the remaining modules
  \end{itemize}
\item \textbf{Load Maven packages} --- resolve coordinates from \code{maven-packages.xml}, same ModuleLayer process
