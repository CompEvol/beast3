...
BEAST 3 core requires **Java 25**. External packages targeting BEAST 3 should also use Java 25.

> **Note:** BEAST 3 core now uses a Maven build system, but external packages may continue to use Ant.

It is recommended to install the JDK from Azul. It can be downloaded from here: https://www.azul.com/downloads/?package=jdk#zulu

* Scroll to the bottom
* Select **Java 25**
* Select your operating system
* Select **JDK** (JavaFX is no longer bundled — it is resolved as a Maven dependency)
* Download the file and install according to instructions under the `How to install` link.

In the IDE that you are using, make sure that the JDK points to Java 25.
...
```
export JAVA_HOME=/path/to/zulu25-jdk25
...
* antlr => antlr-runtime-4.13.2.jar
...
The `PackageHealthChecker` tool comes with BEAST and is best run through an IDE via the `beastfx.app.tools.PackageHealthChecker` class. It performs some checks on the packages, including suggestions on what services to put in the `version.xml` file.
