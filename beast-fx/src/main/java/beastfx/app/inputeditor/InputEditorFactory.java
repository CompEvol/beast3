...
//        registerInputEditors(knownEditors);
     ...
     */
    public List<InputEditor> addInputs(VBox box, BEASTInterface beastObject, InputEditor editor, InputEditor validateListener, BeautiDoc doc) {
        ...
        for (Input<?> input : inputs) {
            try {
                ...
                e.printStackTrace();
                Alert.showMessageDialog(null, "Could not add entry for " + input.getName());
            }
        }
        // box.add(Box.createVerticalGlue());
        return editors;
