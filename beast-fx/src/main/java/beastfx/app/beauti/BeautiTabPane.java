...
public class BeautiTabPane extends beastfx.app.inputeditor.BeautiTabPane implements BeautiDocListener {
    ...
    /**
     * Load a BEAST analysis XML file, replacing the analysis currently in BEAUti.
     * Shared by the File/Load menu and by dragging and dropping an XML file on BEAUti.
     */
    ...
    /**
     * Accept files dropped anywhere on BEAUti. Only BEAST analysis XML files are handled
     * here -- alignment files are handled by the partitions table in AlignmentListInputEditor,
     * which gets the event first and consumes it.
     */
    public void installDragAndDropHandler(Scene scene) {
        ...
        scene.setOnDragDropped(event -> {
            ...
            if (db.hasFiles()) {
                success = loadDroppedAnalyses(db.getFiles());
                if (!success) {
                    // do not leave the drop unanswered, but wait for the drag to finish first
                    Platform.runLater(() -> Alert.showMessageDialog(null,
                            "Only BEAST XML files generated by BEAUti can be loaded by dropping them on BEAUti. "
                            + "Drop alignment files on the table in the Partitions panel to import them."));
                }
            }
    ...
    /**
     * Load the first BEAST analysis XML file among a set of dropped files, after confirming
     * that the current analysis may be replaced. Files that are not BEAUti generated analyses
     * are ignored.
     *
     * @return true if one of the files was accepted for loading
     */
    public boolean loadDroppedAnalyses(List<File> files) {
        List<File> analyses = new ArrayList<>();
        for (File file : files) {
            if (BeautiDoc.isBeautiAnalysisFile(file)) {
                analyses.add(file);
            }
        }
        if (analyses.size() == 0) {
        ...
        final File analysis = analyses.get(0);
        final boolean ignoredSomeFiles = files.size() > 1;
        final boolean replacesAnalysis = doc.alignments.size() > 0;

        // loading an analysis replaces every panel in BEAUti, including the one the drop
        // event originated from, so let the drag and drop finish before doing anything
        Platform.runLater(() -> {
            if (ignoredSomeFiles) {
                Alert.showMessageDialog(null, "Loading a BEAST analysis replaces everything else in BEAUti, "
                        + "so only " + analysis.getName() + " will be loaded.");
            }
            if (replacesAnalysis) {
                ButtonType choice = Alert.showConfirmDialog(null,
                        "Loading " + analysis.getName() + " will replace the current analysis. Continue?",
                        "Load BEAST analysis",
                        Alert.OK_CANCEL_OPTION);
                if (choice != Alert.OK_OPTION) {
                    return;
                }
            }
            loadFile(analysis);
...
//	        		Tooltip.install(menu.getContent(), tooltip);
    ...
    public static BeautiTabPane main2(String[] args, Stage primaryStage, BeautiDoc doc) {
        ...
        try {
            ...
            beauti.installDragAndDropHandler(scene);
